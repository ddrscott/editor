...
/**
 * MySQL code executor using db-api.
 * Runs MySQL queries via the db-api service with real MySQL 8 containers.
 */
export class MySqlRunner implements LanguageRunner {
  readonly language = 'mysql';
  readonly supportedExtensions = ['mysql'];
  ...
  private async _initialize(): Promise<void> {
    ...
    const existingId = syncClient?.getDbInstanceId('mysql');
    if (existingId) {
      ...
      try {
        ...
        console.log('MySQL instance expired, creating new one');
      }
    }

    // Create new database instance
    try {
      const result = await this.dbClient.createDb('mysql');
      this.dbInstanceId = result.db_id;

      // Store in Durable Object for collaboration
      syncClient?.sendDbStatus('mysql', this.dbInstanceId, 'ready');

      this.initialized = true;
    } catch (error) {
      this.initializing = null;
      throw new Error(`Failed to create MySQL database: ${error}`);
    }
  }
    ...
    try {
      ...
      if (errorMsg.includes('not found') || errorMsg.includes('expired') || errorMsg.includes('404')) {
        ...
        return {
          success: false,
          output: '',
          error: 'MySQL database expired. Run again to create a new instance.',
          executionTime,
        };
      }
